...
def crate_binary(name, crate_root, lib = None, srcs = None, tests = True, **kwargs):
    """`rust_binary` for a `[[bin]]` target, with or without a library beside it.

    Args:
      name: the binary target name, matching Cargo's `[[bin]] name`.
      crate_root: the binary's entry point, e.g. `src/bin/broker.rs`.
      lib: the `crate_library` target in this package that it links. Omit for a
        crate that is only a binary -- one with no `lib.rs` -- where the
        modules belong to the binary itself and there is no library to link.
      srcs: sources; defaults to `[crate_root]` when a `lib` carries the rest,
        and to `src/**/*.rs` when there is none.
    ...
    """
    if srcs == None:
        srcs = [crate_root] if lib else native.glob(["src/**/*.rs"])
...
def crate_tests(
    ...
    Args:
      lib: the `crate_library` target name in this package. A binary-only
        crate passes its `crate_binary` name instead: `rust_test` over a binary
        compiles it with `--test` just as it does a library.
