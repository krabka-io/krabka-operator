...
//! `krabka.io/cluster=<this name>`. The `Ready` condition follows this
//! rule:
...
pub(crate) fn rollup_condition(rollup: &ClusterRollup) -> (bool, &'static str, String) {
    if rollup.pool_count == 0 {
        (
            false,
            "NoNodePools",
            "no KafkaNodePool with label krabka.io/cluster=<name>".into(),
        )
...
/// from the `krabka.io/cluster` label and the namespace from the pool
/// itself.
///
...
/// Patches every pool with the label `krabka.io/cluster=<this Kafka>`.
///
/// The patch sets `metadata.ownerReferences`, so that the Kafka is the
/// controlling owner, AND
/// `metadata.labels["krabka.io/config-hash"]`, so that the pool
    ...
    let states: Vec<common::PoolRolloutState> = ordered
        ...
        .map(|p| common::PoolRolloutState {
            ...
            current_hash: p
                ...
                .labels
                .as_ref()
                .and_then(|l| l.get("krabka.io/config-hash").cloned()),
            ready: p
                .status
                .as_ref()
    ...
    for (pool_name, target_hash) in plan {
        ...
        let patch_body = json!({
            ...
            "metadata": {
                ...
                "labels": { "krabka.io/config-hash": target_hash },
            }
        });
        pool_api
...
/// `krabka.io/config-hash` label and its live `StatefulSet` has observed that
...
pub(crate) fn pools_converged<'a, 'b>(
    ...
    for p in pools {
        ...
        let template_hash = spec
            ...
            .and_then(|annotations| annotations.get("krabka.io/config-hash"));
...
mod tests {
    ...
    fn ca_rollout_gate_rejects_stale_ready_revision_then_accepts_convergence() {
        ...
        p.metadata.labels = Some(BTreeMap::from([
            ("krabka.io/config-hash".into(), "new-hash".into()),
            ("krabka.io/cluster".into(), "demo".into()),
        ]));
        ...
        statefulset.spec = Some(
            serde_json::from_value(json!({
                ...
                "template": {
                    "metadata": { "annotations": { "krabka.io/config-hash": "new-hash" } },
                    "spec": { "containers": [] }
                }
            }))
            .unwrap(),
        ...
        p.metadata
            ...
            .as_mut()
            .unwrap()
            .insert("krabka.io/config-hash".into(), String::new());
        assert!(!pools_converged([&p], [&statefulset]));
    }
