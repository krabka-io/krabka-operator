...
//!   provisions its ACLs.
//!
//! The reconciler finds the parent `Kafka` from the `krabka.io/cluster`
...
fn gateway_labels(parent_name: &str, gw_name: &str) -> BTreeMap<String, String> {
    ...
    m.insert("krabka.io/gateway".into(), gw_name.into());
    m
}
...
/// the `krabka.io/cluster` label, and its owner reference points to the
/// gateway CR.
///
...
fn child_kafkauser(gw: &KafkaGrpcGateway, parent_name: &str) -> Result<KafkaUser, ReconcileError> {
    ...
    labels.insert("krabka.io/cluster".into(), parent_name.to_string());
    labels.insert("krabka.io/gateway".into(), gw_name.clone());
...
async fn reconcile_inner(
    ...
    // (1) Parse the `krabka.io/cluster` label → fetch the parent Kafka.
    let Some(parent_name) = gw
        ...
        let cond = condition(
            ...
            "False",
            "MissingClusterLabel",
            "metadata.labels.\"krabka.io/cluster\" is required to link a gateway to its parent Kafka",
        );
...
mod tests {
    ...
    fn deployment_name_and_owner_ref() {
        ...
        let expected = OwnerReference {
            api_version: "krabka.io/v1alpha1".into(),
    ...
    fn child_kafkauser_is_tls_with_broad_acls() {
        ...
        // krabka.io/cluster label links it to the parent.
        check!(
            user.metadata
                .labels
                .as_ref()
