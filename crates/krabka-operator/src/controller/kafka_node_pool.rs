...
pub enum PoolValidationError {
    ...
    NoControllerNodes,
    #[error("metadata.labels.\"krabka.io/cluster\" missing")]
    MissingClusterLabel,
    ...
    statefulsets
        ...
        .filter(|statefulset| {
            statefulset
                ...
                .labels
                .as_ref()
                .and_then(|labels| labels.get("krabka.io/pool"))
                == Some(&pool_name)
        })
...
fn pool_labels_match(labels: Option<&BTreeMap<String, String>>, pool_name: &str) -> bool {
    labels
        .and_then(|values| values.get("krabka.io/pool"))
...
pub(crate) fn render_statefulset(
    ...
    selector.insert("krabka.io/pool".into(), pool_name.clone());
...
fn condition_for_validation_error(err: &PoolValidationError) -> KafkaCondition {
    let (reason, message) = match err {
        ...
        PoolValidationError::MissingClusterLabel => (
            "MissingClusterLabel",
            "metadata.labels.\"krabka.io/cluster\" missing".to_string(),
        ),
...
async fn reconcile_inner(
    ...
    // 2. Look up the parent Kafka via the `krabka.io/cluster` label.
    let Some(kafka_name) = pool
        .meta()
        .labels
        .as_ref()
        .and_then(|l| l.get("krabka.io/cluster").cloned())
    else {
        let cond = condition(
            "Ready",
            "False",
            "MissingClusterLabel",
            "metadata.labels.\"krabka.io/cluster\" is required to link a pool to its parent Kafka",
        );
        patch_status_for_pool(&pool_api, &name, cond).await?;
        return Ok(Action::await_change());
    };
    ...
    let siblings = pool_api
        .list(&ListParams::default().labels(&format!("krabka.io/cluster={kafka_name}")))
        .await?;
...
mod tests {
    ...
    fn topology_reserves_node_ids_while_old_statefulset_replicas_are_live() {
        ...
        observed.metadata.labels = Some(
            [("krabka.io/pool".to_string(), "shrinking".to_string())]
                .into_iter()
                .collect(),
        );
    ...
    fn topology_rejects_node_id_start_and_role_mutation_from_live_identity() {
        let mut observed = StatefulSet::default();
        observed.metadata.labels = Some(
            [("krabka.io/pool".to_string(), "controllers".to_string())]
                .into_iter()
                .collect(),
        );
    ...
    fn topology_reserves_node_ids_for_surviving_pod_ordinals() {
        ...
        pod.metadata.labels = Some(
            [("krabka.io/pool".to_string(), "first".to_string())]
                .into_iter()
                .collect(),
        );
    ...
    fn render_statefulset_template_annotations_apply() {
        ...
        annos.insert("krabka.io/test-anno".into(), "yes".into());
        let pool = pool_with_template(PodTemplate {
            metadata: Some(MetadataTemplate {
                labels: BTreeMap::new(),
                annotations: annos,
        ...
        let anno = sts
            ...
            .annotations
            .unwrap();
        assert!(anno.get("krabka.io/test-anno").map(String::as_str) == Some("yes"));
    }
    ...
    fn render_statefulset_propagates_config_hash_from_label() {
        ...
        pool.metadata
            ...
            .insert("krabka.io/config-hash".into(), "abc123".into());
        let sts = render_statefulset(&parent_fixture("demo"), &pool, "img:1").unwrap();
        let anno = sts
            .spec
            .unwrap()
            ...
            .annotations
            .unwrap();
        assert!(anno.get("krabka.io/config-hash").map(String::as_str) == Some("abc123"));
    }
    ...
    fn render_statefulset_no_config_hash_when_label_absent() {
        ...
        if let Some(anno) = sts.spec.unwrap().template.metadata.unwrap().annotations {
            assert!(!anno.contains_key("krabka.io/config-hash"));
        }
    }
    ...
    fn render_statefulset_pvc_labels_inherit_pod_labels() {
        ...
        assert!(labels.get("krabka.io/pool").map(String::as_str) == Some("brokers"));
    }
