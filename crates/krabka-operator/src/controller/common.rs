...
pub(crate) fn common_labels(
    ...
    if let Some(p) = pool {
        m.insert("krabka.io/pool".into(), p.into());
    }
    m
}
...
/// Render the stable controller bootstrap Service. Only Ready controller
/// pods become endpoints, so a single DNS name provides failover without
/// making a parallel cold start dial controllers that are not listening yet.
pub(crate) fn render_controller_bootstrap_service(
    owner: &Kafka,
) -> Result<Service, ReconcileError> {
    let name = owner.meta().name.clone().unwrap_or_default();
    let labels = common_labels(&name, &owner.spec.kafka_version, None);
    let selector: BTreeMap<String, String> = BTreeMap::from([
        ("app.kubernetes.io/name".into(), APP_LABEL.into()),
        ("app.kubernetes.io/instance".into(), name.clone()),
        ("krabka.io/controller-role".into(), "true".into()),
    ]);
    Ok(serde_json::from_value(json!({
        "metadata": {
            "name": format!("{name}-controller-bootstrap"),
            "namespace": owner.meta().namespace.clone(),
            "labels": labels,
            "ownerReferences": [owner_ref::<Kafka>(owner)?],
        },
...
pub(crate) fn render_configmap(
    ...
    let controller_endpoints: Vec<(i32, String)> = addresses_per_broker
        ...
        .collect();
    // A ClusterIP Service routes to whichever controller pods are Ready. It
    // gives joiners failover without listing not-yet-listening cold-start pods.
    let controller_bootstrap_servers = vec![format!(
        "{name}-controller-bootstrap.{}.svc.cluster.local:{CONTROLLER_PORT}",
...
/// the pool's `krabka.io/config-hash` label (`None` if never stamped);
...
mod cluster_object_tests {
    ...
    #[test]
    fn controller_bootstrap_service_selects_ready_controllers() {
        let svc = render_controller_bootstrap_service(&test_kafka()).expect("service");
        let spec = svc.spec.expect("spec");
        assert!(spec.cluster_ip.is_none());
        assert!(
            spec.selector
                .expect("selector")
                .get("krabka.io/controller-role")
                .map(String::as_str)
                == Some("true")
        );
        assert!(spec.publish_not_ready_addresses.is_none());
        assert!(spec.ports.expect("ports")[0].port == CONTROLLER_PORT);
    }
    ...
    fn configmap_wires_controllers_and_broker_observer() {
        ...
        let expected =
            "bootstrap_servers = [\"demo-controller-bootstrap.default.svc.cluster.local:9093\"]";
