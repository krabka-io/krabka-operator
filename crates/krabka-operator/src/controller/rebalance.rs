...
const ANNOTATION: &str = "krabka.io/rebalance";
...
/// The pending command from the `krabka.io/rebalance` annotation, if any.
#[must_use]
        ...
        Ok(None) => {
            write_status(
                ...
                &Outcome::transient(
                    RebalanceState::NotReady,
                    "MissingEndpoint",
                    "spec.endpoint is unset and no krabka.io/cluster label is present to derive it"
                        .into(),
...
mod tests {
    ...
    fn read_command_parses_annotation() {
        ...
        k.metadata.annotations = Some(
            [("krabka.io/rebalance".to_string(), "approve".to_string())]
                .into_iter()
                .collect(),
        );
    ...
    fn read_command_none_for_unknown_value() {
        let mut k = cr("x");
        k.metadata.annotations = Some(
            [("krabka.io/rebalance".to_string(), "yolo".to_string())]
                .into_iter()
                .collect(),
        );
    ...
    fn resolve_endpoint_derives_from_cluster_label() {
        ...
        k.metadata.labels = Some(
            [("krabka.io/cluster".to_string(), "demo".to_string())]
                .into_iter()
                .collect(),
        );
        assert!(
