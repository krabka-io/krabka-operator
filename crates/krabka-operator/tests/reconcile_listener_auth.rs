...
/// `krabka.io/config-hash` label on the pool owner-ref PATCH.
#[tokio::test]
async fn auth_change_bumps_config_hash() {
    ...
    let hash1 = body1["metadata"]["labels"]["krabka.io/config-hash"]
        .as_str()
    ...
    let hash2 = body2["metadata"]["labels"]["krabka.io/config-hash"]
        .as_str()
