...
fn topic(name: &str, ns: &str, cluster: Option<&str>) -> KafkaTopic {
    ...
    if let Some(c) = cluster {
        ...
        labels.insert("krabka.io/cluster".into(), c.into());
        kt.metadata.labels = Some(labels);
    }
    kt
}

/// A `KafkaTopic` with no `krabka.io/cluster` label must surface
...
async fn finalizer_add_path_patches_metadata_and_requeues_immediately() {
    ...
    assert!(
        finalizers == &serde_json::json!(["krabka.io/topic-finalizer"]),
...
fn topic_with_finalizer(
    ...
    kt.metadata.finalizers = Some(vec!["krabka.io/topic-finalizer".into()]);
    let mut labels = std::collections::BTreeMap::new();
    labels.insert("krabka.io/cluster".into(), CLUSTER.into());
    kt.metadata.labels = Some(labels);
    kt
}
