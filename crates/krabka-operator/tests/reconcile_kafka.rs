...
fn happy_path_rules(
    ...
    //    `krabka.io/cluster=<this>`. Without these owner-refs, deleting
...
async fn kafka_patches_pool_label_with_config_hash() {
    ...
    let hash = body["metadata"]["labels"]["krabka.io/config-hash"]
        .as_str()
        .unwrap_or_else(|| {
            panic!("expected metadata.labels[krabka.io/config-hash] str, body = {body}")
        });
    check!(
        hash.len() == 16,
