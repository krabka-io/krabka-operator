    ...
    json!({
        ...
        "items": [{
            ...
            "metadata": {
                ...
                "namespace": ns,
                "uid": format!("{name}-uid"),
                "labels": { "krabka.io/cluster": cluster }
            },
...
async fn cluster_ca_within_renewal_window_renews_same_key() {
    ...
    rules.extend([
        ...
        get(
            ...
            &secret_with(
                ...
                &[("krabka.io/ca-cert-generation", "0")],
            ),
        ),
        patch(
            format!("/secrets/{c}-cluster-ca-cert"),
    ...
    check!(
        body["metadata"]["annotations"]["krabka.io/ca-cert-generation"] == "1",
...
async fn force_replace_key_starts_staged_rotation() {
    ...
    reconcile(
        Arc::new(kafka_cr(
            c,
            ns,
            &[("krabka.io/force-replace-ca-key", "true")],
        )),
        ctx,
    )
    .await
    ...
    assert!(
        cbody["metadata"]["annotations"]["krabka.io/ca-rotation-phase"] == "key-replace-trust",
    ...
    assert!(
        mbody["metadata"]["annotations"]["krabka.io/force-replace-ca-key"].is_null(),
...
async fn force_replace_clients_key_starts_trust_distribution() {
    ...
    reconcile(
        Arc::new(kafka_cr(
            c,
            ns,
            &[("krabka.io/force-replace-clients-ca-key", "true")],
        )),
        ctx,
    )
    .await
    ...
    assert!(body["metadata"]["annotations"]["krabka.io/ca-rotation-phase"] == "key-replace-trust");
    assert!(
        !observed
            .iter()
...
async fn clients_key_promotion_reissues_users_before_marking_converged() {
    ...
    pool["metadata"]["labels"]["krabka.io/config-hash"] = json!("trust-roll-complete");
    ...
    rules.extend([
        ...
        get(
            ...
            &secret_with(
                ...
                &[
                    ("krabka.io/ca-cert-generation", "0"),
                    ("krabka.io/ca-rotation-phase", "key-replace-trust"),
                ],
            ),
        ),
        patch(
    ...
    assert!(marker_body["metadata"]["annotations"]["krabka.io/ca-leafs-key-generation"] == "1");
    assert!(state.remaining_rules() == 0, "all rules consumed");
}
...
async fn clients_ca_prune_removes_old_root_from_user_secrets_before_returning() {
    ...
    let mut pool = fake_pool_list_item("brokers", ns, c, 1, 1);
    pool["metadata"]["labels"]["krabka.io/config-hash"] = json!("promote-roll-complete");
    let statefulset = fake_converged_sts_body(
        &format!("{c}-brokers"),
        ns,
        c,
    ...
    rules.extend([
        ...
        get(
            ...
            &secret_with(
                ...
                &[("ca.crt", &bundle)],
                &[
                    ("krabka.io/ca-cert-generation", "1"),
                    ("krabka.io/ca-key-generation", "1"),
                    ("krabka.io/ca-leafs-key-generation", "1"),
                    ("krabka.io/ca-rotation-phase", "key-replace-promote"),
                ],
            ),
        ),
        patch(
