...
///      `krabka.io/ca-renew-after` on it.
...
async fn cronjob_flags_expiring_cluster_ca_without_rotating() {
    ...
    assert!(
        meta_body["metadata"]["annotations"]["krabka.io/ca-renew-after"].is_string(),
        "metadata PATCH must stamp krabka.io/ca-renew-after; body = {meta_body}",
    );
